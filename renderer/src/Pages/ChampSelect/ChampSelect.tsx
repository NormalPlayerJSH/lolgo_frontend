...
export default function ChampSelect(props:{UserInfo:UserInfoI}) {
  ...
  const { data: PickData, isValidating } = useSWR('/lol-champ-select/v1/session', (url) => axios.get<any>(url).then((res) => res.data), {
    refreshInterval: 250,
    dedupingInterval: 0,
  });
    ...
    (async () => {
      while (true) {
        try {
          ...
          // eslint-disable-next-line no-await-in-loop
          const { data: myTeamInfoData } = await axios.get<any>(`/lol-summoner/v2/summoners/?ids=${JSON.stringify(myTeamSummonerIds)}`);
